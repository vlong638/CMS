namespace WpfApplication2.Business
{
	...
	/// 线路速度表业务逻辑类
	/// </summary>
	public class SpeedInfoManager : BizBase
	{
		#region Fields
		private SpeedInfoDAL _dal;
		#endregion

		#region Constructors

		/// <summary>
		/// 初始化 <see cref="SpeedInfoManager" /> 类的新实例。
		/// </summary>
		/// <param name="session"></param>
		public SpeedInfoManager(ISession session)
			: base(session)
		{
			_dal = new SpeedInfoDAL(session);
		}

		/// <summary>
		/// 初始化 <see cref="SpeedInfoManager" /> 类的新实例。
		/// </summary>
		public SpeedInfoManager()
		{
			_dal = new SpeedInfoDAL(base.Session);
		}
        ...
        /// <param name="dao">数据对象</param>
        /// <returns>成功/失败</returns>
        public bool Insert(List<SpeedInfo> daos)
        {
            bool ret = true;
            try
            {
