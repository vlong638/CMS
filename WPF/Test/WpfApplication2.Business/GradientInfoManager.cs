namespace WpfApplication2.Business
{
	...
	/// 线路坡度表业务逻辑类
	/// </summary>
	public class GradientInfoManager : BizBase
	{
		#region Fields
		private GradientInfoDAL _dal;
		#endregion

		#region Constructors

		/// <summary>
		/// 初始化 <see cref="GradientInfoManager" /> 类的新实例。
		/// </summary>
		/// <param name="session"></param>
		public GradientInfoManager(ISession session)
			: base(session)
		{
			_dal = new GradientInfoDAL(session);
		}

		/// <summary>
		/// 初始化 <see cref="GradientInfoManager" /> 类的新实例。
		/// </summary>
		public GradientInfoManager()
		{
			_dal = new GradientInfoDAL(base.Session);
		}
        ...
        /// <param name="dao">数据对象</param>
        /// <returns>成功/失败</returns>
        public bool Insert(List<GradientInfo> daos)
        {
            bool ret = true;
            try
            {
