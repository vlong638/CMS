namespace WpfApplication2.Business
{
	...
	/// 制动距离信息表业务逻辑类
	/// </summary>
	public class BrakingInfoManager : BizBase
	{
		#region Fields
		private BrakingInfoDAL _dal;
		#endregion

		#region Constructors

		/// <summary>
		/// 初始化 <see cref="BrakingInfoManager" /> 类的新实例。
		/// </summary>
		/// <param name="session"></param>
		public BrakingInfoManager(ISession session)
			: base(session)
		{
			_dal = new BrakingInfoDAL(session);
		}

		/// <summary>
		/// 初始化 <see cref="BrakingInfoManager" /> 类的新实例。
		/// </summary>
		public BrakingInfoManager()
		{
			_dal = new BrakingInfoDAL(base.Session);
		}
		...
		/// <param name="rowsCount">总记录条数</param>
		/// <returns>分页数据</returns>
		public ObservableCollection<BrakingInfo> SelectAllCollection(int pageIndex, int pageSize, int rowsCount)
		{
