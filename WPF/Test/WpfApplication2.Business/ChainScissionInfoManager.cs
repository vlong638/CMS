namespace WpfApplication2.Business
{
	...
	/// 断链信息表业务逻辑类
	/// </summary>
	public class ChainScissionInfoManager : BizBase
	{
		#region Fields
		private ChainScissionInfoDAL _dal;
		#endregion

		#region Constructors

		/// <summary>
		/// 初始化 <see cref="ChainScissionInfoManager" /> 类的新实例。
		/// </summary>
		/// <param name="session"></param>
		public ChainScissionInfoManager(ISession session)
			: base(session)
		{
			_dal = new ChainScissionInfoDAL(session);
		}

		/// <summary>
		/// 初始化 <see cref="ChainScissionInfoManager" /> 类的新实例。
		/// </summary>
		public ChainScissionInfoManager()
		{
			_dal = new ChainScissionInfoDAL(base.Session);
		}
        ...
        /// <param name="dao">数据对象</param>
        /// <returns>成功/失败</returns>
        public bool Insert(List<ChainScissionInfo> daos)
        {
            bool ret = true;
            try
            {
		...
		/// <param name="rowsCount">总记录条数</param>
		/// <returns>分页数据</returns>
		public ObservableCollection<ChainScissionInfo> SelectAllCollection(int pageIndex, int pageSize, int rowsCount)
		{
