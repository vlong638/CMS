namespace WpfApplication2.Business
{
	...
	/// 坐标系信息表业务逻辑类
	/// </summary>
	public class CoordinateInfoManager : BizBase
	{
		#region Fields
		private CoordinateInfoDAL _dal;
		#endregion

		#region Constructors

		/// <summary>
		/// 初始化 <see cref="CoordinateInfoManager" /> 类的新实例。
		/// </summary>
		/// <param name="session"></param>
		public CoordinateInfoManager(ISession session)
			: base(session)
		{
			_dal = new CoordinateInfoDAL(session);
		}

		/// <summary>
		/// 初始化 <see cref="CoordinateInfoManager" /> 类的新实例。
		/// </summary>
		public CoordinateInfoManager()
		{
			_dal = new CoordinateInfoDAL(base.Session);
		}
        ...
        /// <param name="dao">数据对象</param>
        /// <returns>成功/失败</returns>
        public bool Insert(List<CoordinateInfo> daos)
        {
            bool ret = true;
            try
            {
		...
		/// <param name="rowsCount">总记录条数</param>
		/// <returns>分页数据</returns>
		public ObservableCollection<CoordinateInfo> SelectAllCollection(int pageIndex, int pageSize, int rowsCount)
		{
